...
export default function ApplicationDetailsPage({ params }) {
  ...
  async function handleAddStageEntry() {
    await addStageEntry({ id, status: stageStatus, notes: stageNotes });
    setModal(null);
    setStageNotes('');
  }
    ...
    <>
      <div className="page-header flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          {app.status === 'blocked_90d' && (
            <button className="btn btn-primary" onClick={() => setModal('reapprove')}>Request Re-Approval</button>
          )}
          {app.status === 'approved' && (
            <button className="btn btn-primary" onClick={() => setModal('disburse')}>Mark as Disbursed</button>
          )}
        </div>
      </div>
      ...
      {modal === 'disburse' && (
        <div className="modal-backdrop" onClick={() => setModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Mark Loan as Disbursed</h3>
            <p className="text-sm text-muted mb-4">This will finalize the loan and automatically generate any applicable dealer commissions.</p>
            <div className="field">
              <label>Bank UTR Number *</label>
              <input className="input" value={utr} onChange={e => setUtr(e.target.value)} placeholder="e.g. HDFC123456789" />
            </div>
            <div className="flex justify-end gap-2 mt-6">
              <button className="btn btn-secondary" onClick={() => setModal(null)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleDisburse} disabled={disbursing || !utr}>{disbursing ? 'Processing...' : 'Confirm Disbursement'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'reapprove' && (
        <div className="modal-backdrop" onClick={() => setModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Request Re-Approval</h3>
            <p className="text-sm text-muted mb-4">This loan was blocked due to 90 days of inactivity. Re-approving it will revert its status to 'approved' and reset the disbursement timer.</p>
            <div className="flex justify-end gap-2 mt-6">
              <button className="btn btn-secondary" onClick={() => setModal(null)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleReApprove} disabled={reApproving}>{reApproving ? 'Processing...' : 'Confirm Re-Approval'}</button>
            </div>
          </div>
        </div>
      )}
