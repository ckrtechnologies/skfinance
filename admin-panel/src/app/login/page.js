...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Store token
      localStorage.setItem('sk_admin_token', data.data.token);
      router.push('/');
    } catch (err) {
    ...
    <div style={{ display: 'flex', height: '100dvh', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg)' }}>
      <div className="card" style={{ width: 400, maxWidth: '90%', padding: '40px 32px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-text)' }}>Admin Login</h1>
          <p style={{ fontSize: 13, color: 'var(--color-text-3)', marginTop: 4 }}>Sign in to SK Finance admin portal</p>
        </div>
