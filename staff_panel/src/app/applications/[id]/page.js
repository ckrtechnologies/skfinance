...
import { useGetApplicationQuery, useGetStageEntriesQuery, useAddStageEntryMutation, useGetLendersQuery } from '@/store/api/staffApi';
...
export default function ApplicationDetailsPage({ params }) {
  ...
  async function handleAddStageEntry() {
    ...
    if (stageAction === 'clarification') {
      ...
      outcome = 'approved';
      if (targetStage === 'approval') newStatus = 'approved';
            ...
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px' }}>
                ...
                <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', padding: '12px', borderRadius: '10px' }}>
                  <span style={{ fontSize: '10px', fontWeight: 800, color: '#10B981', textTransform: 'uppercase' }}>Approved Loan Amount</span>
                  <div style={{ fontSize: '18px', fontWeight: 800, color: '#10B981', marginTop: '2px' }}>
                    <AmountCell value={app.approved_amount || app.requested_amount} />
                  </div>
                </div>
                <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', padding: '12px', borderRadius: '10px' }}>
                  <span style={{ fontSize: '10px', fontWeight: 800, color: '#3B82F6', textTransform: 'uppercase' }}>Sanctioning Bank / Lender</span>
                  <div style={{ fontSize: '14px', fontWeight: 800, color: '#3B82F6', marginTop: '4px' }}>
                    🏦 {app.lenders?.name || 'HDFC Bank'}
                  </div>
                </div>
                <div style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', padding: '12px', borderRadius: '10px' }}>
